...
const app = express();
app.use(bodyParser.json()); //Body parser é o responsável por criar o json
...
app.put('/veiculos/:placa', async (req, res) => {
    ...
    const [updated] = await req.Veiculo.update({ marca, modelo, ano }, { where: { placa } });
    if (updated) {
        const updatedVeiculo = await req.Veiculo.findOne({ where: { placa } });
...
app.delete('/veiculos/:placa', async (req, res) => {
    const { placa } = req.params;
    const deleted = await req.Veiculo.destroy({ where: { placa } });
    if (deleted) {
